import type { Answer, Field } from "@/lib/form-model/types";

/**
 * Ported from main's packages/conversation/src/announce.ts, unmodified logic.
 * Announcements are the engine's only output. The engine never speaks and
 * never touches the DOM — the presentation layer decides whether an
 * announcement becomes an ARIA live region update, a spoken utterance, or
 * both, depending on whether a screen reader is present.
 */
