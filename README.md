...
This repository's `main` branch is the real, working codebase. The AI/ingest pipeline (this doc, `docs/INTEGRATION.html`, `src/lib/ingest/`, `src/lib/openrouter/`) is the core of it. An earlier `packages/*` npm-workspace scaffold from the original architecture plan was never completed; it is excluded from typecheck/lint (see `tsconfig.json`, `eslint.config.mjs`) and kept on disk for reference only. The real app is entirely in `src/`.
...
Deployed at [formfill-main.vercel.app](https://formfill-main.vercel.app) (Vercel project `formfill-main`, linked to this repo's `main` branch). `OPENROUTER_API_KEY` is set as a Production environment variable on the Vercel project; update it via the Vercel dashboard or `vercel env` if it changes. To deploy a new build from `main`:
