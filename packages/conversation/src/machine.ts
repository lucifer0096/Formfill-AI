import {
  ...
  type Field,
  type Form,
} from '@formfill/form-model';
import { validate } from '@formfill/validate';
import {
  confirmationText,
  questionText,
  say,
 ...
 * product (docs/DESIGN.md §3), and keeping it a pure reducer is what makes it
 * possible to snapshot-test exactly what a user will hear.
 *
 * Dependency rule: this package must never import an ingest or emit package.
 */
 ...
 * never hide the original wording (docs/ACCESSIBILITY.md §7).
 */
...
function confirm(state: ConversationState): StepResult {
  ...
  // THE REVIEW GATE (docs/DESIGN.md §4.4). Confirming without having heard every
  // answer read back is the failure this product exists to prevent — the user
  // cannot see the form, so the read-back is their only check on it.
  const fields = applicableFields(state.form, state.answers);
